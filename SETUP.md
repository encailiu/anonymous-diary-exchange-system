...
3. Apps Script左側の「トリガー」で次の2件を確認します。
   ...
   - 時間主導型、毎日22時台: `runDailyExchange`

設定メニューを再実行すると対象の2トリガーを作り直すため、重複して増えません。Apps Scriptの時間トリガーは指定時刻の近辺で実行されるため、厳密に22:05:00に開始するとは限りません。
...
| アーカイブが停止 | フォルダが「制限付き」で、個別共有先が管理者だけか確認する。 |
