...
> [!IMPORTANT]
> `Participants` には、参加者本人が投稿用Google Formへのログイン・投稿に使用するGoogleアカウントと、完全に同じメールアドレスを登録してください。参加者が複数のGoogleアカウントを持っている場合は、実際に投稿に使うアカウントを本人へ確認します。別のメールアドレスを登録すると、その参加者の投稿は受け付けられません。

1. Spreadsheetの「匿名日記システム」→「参加者を追加」を選びます。
2. 参加者本人へ、投稿用Google Formへのログイン・投稿に使うGoogleアカウントを確認し、そのアカウントと完全に同じメールアドレスを入力します。
3. 参加者全員について繰り返します。
...
利用上の注意には、次の内容を含めてください。

> 管理者へ登録したメールアドレスと同じGoogleアカウントで、投稿用Google Formにログインして投稿してください。別のGoogleアカウントで投稿すると、日記は受け付けられません。
...
| 状況 | 確認すること |
| --- | --- |
| 投稿が `rejected` | 参加者がFormへのログイン・投稿に使ったGoogleアカウントのメールアドレスと、`Participants` のメールアドレスが完全に同じか確認する。併せて `active` を確認する。 |
