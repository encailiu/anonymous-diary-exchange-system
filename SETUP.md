...
無効化後は、過去の日記に発行済みの匿名コメントリンクも受付を停止します。
...
全シートで `diary_date` が基準日より前の行だけを対象にし、基準日当日とそれ以後は対象になりません。22:00以降の投稿は翌日分の `diary_date` で判定します。不正・空欄の日付行は自動削除しません。CSVの内容・SHA-256検証がすべて成功した後だけ削除します。CSV数式として解釈され得る本文には、安全のため先頭にシングルクォートが付きます。
